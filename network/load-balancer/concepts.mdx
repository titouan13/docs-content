...
A high availability (HA) setup is an infrastructure without a single point of failure. It prevents a server failure by adding redundancy to every layer of your architecture. In the case of Scaleway Load Balancer, if there is a failure of the main Load Balancer Instance, a backup Instance is automatically immediately provisioned and configured.
